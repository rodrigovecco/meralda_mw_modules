...
class mwmod_mw_devextreme_util extends mwmod_mw_html_manager_util{
	...
	function add_js_webappjs($manorui){
		...
		$jsman->add_jquery();
		$jsman->add_globalize();
		// Quill is required by the DevExtreme HtmlEditor and must load before dx.all.js.
		$quill=new mwmod_mw_html_manager_item_jsexternal("dxquill","/res/dx/js/dx-quill.min.js");
		$jsman->add_item_by_item($quill);
